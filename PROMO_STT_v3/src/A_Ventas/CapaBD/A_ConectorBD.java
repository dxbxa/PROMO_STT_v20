...
package A_Ventas.CapaBD;

import java.sql.*;
import java.util.*;

public class A_ConectorBD{
...
query+="  TARJETA_CONTACTO,\n";
query+="  NOMBRE_BASE\n";
query+="FROM T_CONTACTO \n";
query+="WHERE CEDULA_CONTACTO IN (\n";
query+="SELECT\n";
query+="MAX(C.CEDULA_CONTACTO) AS CEDULA_CONTACTO\n";
query+="FROM T_CONTACTO C\n";
query+="INNER JOIN T_TELEFONO T\n";
query+="ON C.CEDULA_CONTACTO=T.CEDULA_CONTACTO\n";
query+="WHERE C.CEDULA_CONTACTO LIKE '%"+valorBuscado+"%'\n";
query+="AND T.COD_USUARIO='"+pCOD_USUARIO+"')\n";
...
query+="INNER JOIN T_TELEFONO T\n";
query+="ON C.CEDULA_CONTACTO=T.CEDULA_CONTACTO\n";
query+="WHERE C.TARJETA_CONTACTO LIKE '%"+valorBuscado+"%'\n";
query+="AND T.COD_USUARIO='"+pCOD_USUARIO+"')\n";
...
case 3: 
	...
	while(tokens.hasMoreTokens()){
            if(primero){
                    query+="SELECT \n";
                    query+="MAX(C.CEDULA_CONTACTO) AS CEDULA_CONTACTO\n";
                    query+="FROM T_CONTACTO C INNER JOIN T_TELEFONO T ON C.CEDULA_CONTACTO=T.CEDULA_CONTACTO WHERE T.COD_USUARIO= '"+pCOD_USUARIO+"' AND C.NOMBRE_CONTACTO LIKE UPPER('%"+tokens.nextToken()+"%') \n";
                    primero=false;
            }else{
                    query+="AND C.NOMBRE_CONTACTO LIKE UPPER('%"+tokens.nextToken()+"%') \n";
            }
        }
            query+=")";
...
// //////System.out.println(query);
    ...
    //GUARDA LLAMADA
    public void GuardarLlamada(String pCEDULA,String pTELEFONO,String pPROMESA,String pCOMENTARIO, String pCOD_USUARIO,String pfecha) throws Exception{
            String query="DECLARE\n";
...
String query="SELECT to_char(SYSDATE,'DD/MM/YYYY HH:MI am') as fecha FROM DUAL \n";
...
String resultado="";
       ...
       try {
         ...
         try {
           ...
           try{
             while (rs.next()){
                 resultado=rs.getObject("fecha").toString();                 
                 j++;  
             } 
           }
           finally {
...
String resultado[][]=null;
    ...
    //OBTIENE INFORMACION LUEGO DE INGRESARLA AL FORMULARIO
...
////////System.out.println(query);
    ...
    //ASESOR - VALIDA DUPLICADOS EN FORMULARIO
...
//////System.out.println(query);
    ...
    //OBTIENE INFORMACION PARA EL CD
    public String[][] getCD_PPT(String pfecha)throws Exception{
...
if (!pfecha.equals("")){query+="  AND ENVIO_CD LIKE '"+pfecha+"%'\n";}
...
String resultado[][]=null;
    ...
    public String[][] getCD_CAU(String pfecha)throws Exception{  
String query="SELECT DISTINCT \n";
query+="  --\n";
query+="  U.NOMBRE_USUARIO,\n";
query+="  HORA_VENTA,\n";
...
//////System.out.println(query);
    ...
    //CONTROL - OBTIENE INFORMACION DE FORMULARIOS (+BUSQUEDA)
...
query+="  TELEFONO_CONTACTO,\n";
query+="  CIERRE,\n";
query+="  CASE WHEN ENVIO_CD IS NULL THEN ' ' ELSE ENVIO_CD END AS ENVIO_CD,\n";
query+="  CASE WHEN REVISION_CONTROL_1 IS NULL THEN ' ' ELSE REVISION_CONTROL_1 END AS REVISION_CONTROL_1,\n";
query+="  CASE WHEN STATUS_CONTROL_1 IS NULL THEN ' ' ELSE STATUS_CONTROL_1 END AS STATUS_CONTROL_1,\n";
query+="  CASE WHEN COMENTARIO_CONTROL_1 IS NULL THEN ' ' ELSE COMENTARIO_CONTROL_1 END AS COMENTARIO_CONTROL_1,\n";
query+="  CASE WHEN FECHA_CONTROL_1 IS NULL THEN ' ' ELSE FECHA_CONTROL_1 END AS FECHA_CONTROL_1,\n";
query+="  CASE WHEN REVISION_CONTROL_2 IS NULL THEN ' ' ELSE REVISION_CONTROL_2 END AS REVISION_CONTROL_2,\n";
query+="  CASE WHEN STATUS_CONTROL_2 IS NULL THEN ' ' ELSE STATUS_CONTROL_2 END AS STATUS_CONTROL_2,\n";
query+="  CASE WHEN COMENTARIO_CONTROL_2 IS NULL THEN ' ' ELSE COMENTARIO_CONTROL_2 END AS COMENTARIO_CONTROL_2,\n";
query+="  CASE WHEN FECHA_CONTROL_2 IS NULL THEN ' ' ELSE FECHA_CONTROL_2 END AS FECHA_CONTROL_2,\n";
query+="  CASE WHEN STATUS_ADMIN IS NULL THEN ' ' ELSE STATUS_ADMIN END AS STATUS_ADMIN,\n";
query+="  CASE WHEN FECHA_ADMIN IS NULL THEN ' ' ELSE FECHA_ADMIN END AS FECHA_ADMIN,\n";
query+="  CASE WHEN STATUS_AFILIACIONES IS NULL THEN ' ' ELSE STATUS_AFILIACIONES END AS STATUS_AFILIACIONES,\n";
query+="  CASE WHEN FECHA_AFILIACIONES  IS NULL THEN ' ' ELSE FECHA_AFILIACIONES END AS FECHA_AFILIACIONES\n";
query+="FROM T_F_PRODUCTO_ADICIONAL F\n";
query+="LEFT JOIN T_IDENTIFICADOR IA\n";
query+="ON IA.CEDULA_CONTACTO=F.CEDULA_CONTACTO AND IA.COD_IDENTIFICADOR='PLAN'\n";
query+="LEFT JOIN T_IDENTIFICADOR IB\n";
...
String resultado[][]=null;
         ...
         try {
           ...
           try{
             while (rs.next()){
                 ...
                 resultado[j][12]=rs.getObject("ENVIO_CD").toString();
                 resultado[j][13]=rs.getObject("REVISION_CONTROL_1").toString();
                 resultado[j][14]=rs.getObject("STATUS_CONTROL_1").toString();
                 resultado[j][15]=rs.getObject("COMENTARIO_CONTROL_1").toString();
                 resultado[j][16]=rs.getObject("FECHA_CONTROL_1").toString();
                 resultado[j][17]=rs.getObject("REVISION_CONTROL_2").toString();
                 resultado[j][18]=rs.getObject("STATUS_CONTROL_2").toString();
                 resultado[j][19]=rs.getObject("COMENTARIO_CONTROL_2").toString();
                 resultado[j][20]=rs.getObject("FECHA_CONTROL_2").toString();
                 resultado[j][21]=rs.getObject("STATUS_ADMIN").toString();
                 resultado[j][22]=rs.getObject("FECHA_ADMIN").toString();
                 resultado[j][23]=rs.getObject("STATUS_AFILIACIONES").toString();
                 resultado[j][24]=rs.getObject("FECHA_AFILIACIONES").toString();
    ...
    public String[][] getAuditarPPT_X_cedula(String pcedula)throws Exception{
String query="SELECT DISTINCT \n";
query+="  HORA_VENTA,\n";
query+="  COD_CUENTA,\n";
query+="  TARJETA_CONTACTO,\n";
...
query+="ON IB.CEDULA_CONTACTO=F.CEDULA_CONTACTO AND IB.COD_IDENTIFICADOR='NACIMIENTO'\n";
query+="JOIN T_USUARIO U ON U.COD_USUARIO=F.COD_USUARIO\n";
if (!pcedula.equals("")){query+="  WHERE F.CEDULA_CONTACTO LIKE '%"+pcedula+"%'\n";}
query+="ORDER BY to_date(HORA_VENTA,'DD/MM/YYYY HH:MI AM'),CEDULA_CONTACTO\n";
...
String resultado[][]=null;
         ...
         try {
           ...
           try{
             while (rs.next()){
                 ...
                 resultado[j][8]=rs.getObject("TELEFONO_CONTACTO").toString();
                 resultado[j][9]=rs.getObject("TIPO_PRODUCTO").toString();
                 resultado[j][10]=rs.getObject("ENVIO_CD").toString();
                 resultado[j][11]=rs.getObject("REVISION_CONTROL_1").toString();
                 resultado[j][12]=rs.getObject("STATUS_CONTROL_1").toString();
                 resultado[j][13]=rs.getObject("COMENTARIO_CONTROL_1").toString();
                 resultado[j][14]=rs.getObject("FECHA_CONTROL_1").toString();
                 resultado[j][15]=rs.getObject("REVISION_CONTROL_2").toString();
                 resultado[j][16]=rs.getObject("STATUS_CONTROL_2").toString();
                 resultado[j][17]=rs.getObject("COMENTARIO_CONTROL_2").toString();
                 resultado[j][18]=rs.getObject("FECHA_CONTROL_2").toString();
                 resultado[j][19]=rs.getObject("STATUS_ADMIN").toString();
                 resultado[j][20]=rs.getObject("FECHA_ADMIN").toString();
                 resultado[j][21]=rs.getObject("STATUS_AFILIACIONES").toString();
                 resultado[j][22]=rs.getObject("FECHA_AFILIACIONES").toString();
                 j++;  
             } 
           }
           finally {
    ...
    public String[][] getAuditarCAU_X_CEDULA(String pcedula)throws Exception{
String query="SELECT DISTINCT \n";
query+="  HORA_VENTA,\n";
query+="  U.NOMBRE_USUARIO,\n";
query+="  NOMBRE_CONTACTO,\n";
         ...
         try {
           ...
           try{
             while (rs.next()){
                 ...
                 resultado[j][21]=rs.getObject("ENVIO_CD").toString();
                 resultado[j][22]=rs.getObject("REVISION_CONTROL_1").toString();
                 resultado[j][23]=rs.getObject("STATUS_CONTROL_1").toString();
                 resultado[j][24]=rs.getObject("COMENTARIO_CONTROL_1").toString();
                 resultado[j][25]=rs.getObject("FECHA_CONTROL_1").toString();
                 resultado[j][26]=rs.getObject("REVISION_CONTROL_2").toString();
                 resultado[j][27]=rs.getObject("STATUS_CONTROL_2").toString();
                 resultado[j][28]=rs.getObject("COMENTARIO_CONTROL_2").toString();
                 resultado[j][29]=rs.getObject("FECHA_CONTROL_2").toString();
                 resultado[j][30]=rs.getObject("STATUS_ADMIN").toString();
                 resultado[j][31]=rs.getObject("FECHA_ADMIN").toString();
                 resultado[j][32]=rs.getObject("STATUS_AFILIACIONES").toString();
                 resultado[j][33]=rs.getObject("FECHA_AFILIACIONES").toString();
...
query+="  CASE WHEN FECHA_AFILIACIONES  IS NULL THEN ' ' ELSE FECHA_AFILIACIONES END AS FECHA_AFILIACIONES\n";
query+="FROM T_F_FINANCIAMIENTO\n";
if (!pcedula.equals("")){query+="  WHERE CEDULA_CONTACTO LIKE '%"+pcedula+"%'\n";}
...
String resultado[][]=null;
    ...
    //VENTANA - BUSCA FECHAS PARA COMBOBOXES AUDITORIA
...
//////System.out.println(query);
    ...
    //CONTROL - SALVA LLAMADA AUDITADA
    public void salvaAuditoriaPPT(String pfecha,String pcod_cue,String previsada_por,String pstatus_control,String pcomentario,int ptipo)throws Exception{

String query="DECLARE\n";
query+="  VFECHA VARCHAR2(200);\n";
query+="  VCOD_CUE VARCHAR2(200);\n";
query+="  VREVISION_CONTROL_"+ptipo+" VARCHAR2(200);\n";
query+="  VSTATUS_CONTROL_"+ptipo+" VARCHAR2(200);\n";
query+="  VCOMENTARIO_CONTROL_"+ptipo+" VARCHAR2(200);\n";
query+="  VFECHA_CONTROL_"+ptipo+" VARCHAR2(200);\n";
query+="BEGIN\n";
query+="  VFECHA := '"+pfecha+"';\n";
query+="  VCOD_CUE := '"+pcod_cue+"';\n";
query+="  VREVISION_CONTROL_"+ptipo+" := '"+previsada_por+"';\n";
query+="  VSTATUS_CONTROL_"+ptipo+" := '"+pstatus_control+"';\n";
query+="  VCOMENTARIO_CONTROL_"+ptipo+" := '"+pcomentario+"';\n";
query+="  VFECHA_CONTROL_"+ptipo+" := to_char(SYSDATE,'dd/mm/yyyy hh:mi am');\n";
query+="\n";
query+="  SP_G_AUDIT_"+ptipo+"_PPT(\n";
query+="    VFECHA => VFECHA,\n";
query+="    VCOD_CUE => VCOD_CUE,\n";
query+="    VREVISION_CONTROL_"+ptipo+" => VREVISION_CONTROL_"+ptipo+",\n";
query+="    VSTATUS_CONTROL_"+ptipo+" => VSTATUS_CONTROL_"+ptipo+",\n";
query+="    VCOMENTARIO_CONTROL_"+ptipo+" => VCOMENTARIO_CONTROL_"+ptipo+",\n";
query+="    VFECHA_CONTROL_"+ptipo+" => VFECHA_CONTROL_"+ptipo+"\n";
query+="  );\n";
query+="END;\n";
...
////System.out.println(query);
    ...
    public void salvaAuditoriaCAU(String pfecha,String ptarjeta,String previsada_por,String pstatus_control,String pcomentario,int ptipo)throws Exception{
...
////System.out.println(query);
    ...
    //SUPERVISOR - SALVA RESULTADO CITI
    public void salvaAuditoriaPPT_SUPERVISOR(String pfecha,String pcod_cue,String pstatus_control,int ptipo)throws Exception{

String query="DECLARE\n";
query+="  VFECHA VARCHAR2(200);\n";
query+="  VCOD_CUE VARCHAR2(200);\n";
query+="  VSTATUS_A VARCHAR2(200);\n";
query+="  VFECHA_A VARCHAR2(200);\n";
query+="BEGIN\n";
query+="  VFECHA := '"+pfecha+"';\n";
query+="  VCOD_CUE := '"+pcod_cue+"';\n";
query+="  VSTATUS_A := '"+pstatus_control+"';\n";
query+="  VFECHA_A := to_char(SYSDATE,'dd/mm/yyyy hh:mi am');\n";
query+="\n";
query+="  SP_G_AUDIT_"+ptipo+"_PPT(\n";
query+="    VFECHA => VFECHA,\n";
query+="    VCOD_CUE => VCOD_CUE,\n";
query+="    VSTATUS_A => VSTATUS_A,\n";
query+="    VFECHA_A => VFECHA_A\n";
query+="  );\n";
query+="END;\n";
...
//////System.out.println(query);
    ...
    public void salvaAuditoriaCAU_SUPERVISOR(String pfecha,String ptarjeta,String pstatus_control,int ptipo)throws Exception{
String query="DECLARE\n";
query+="  VFECHA VARCHAR2(200);\n";
query+="  VTARJETA VARCHAR2(200);\n";
query+="  VSTATUS_A VARCHAR2(200);\n";
